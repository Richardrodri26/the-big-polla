...
interface Match {
  ...
  stage: string
  locked: boolean
  liveMinute?: number        // solo cuando state === 'live'
...
### Guardar predicción
```
PredictorOverlay → predictionStore.savePrediction(matchId, score)
...
### `app-store.ts` (sin persist — solo UI)
```ts
interface AppState {
...
2. Extender `src/types/domain.ts` con todos los tipos de dominio
3. Mover datos hardcodeados → `repositories/mock/*.repository.ts`
4. Eliminar `tournament-data.ts` (y `mock-data.ts`)
...
- Autenticación / userId real — la prediction-store usa `"me"` por ahora
- WebSockets / polling para partidos en vivo — fuera de scope
- Gestión de ligas (CRUD) — Fase 6 del roadmap
- `src/app/(main)/oracle/page.tsx` — bracket predictions, dominio separado
