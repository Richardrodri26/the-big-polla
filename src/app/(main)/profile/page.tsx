...
export default async function ProfilePage() {
  ...
  const [matches, members, badges] = await Promise.all([
    ...
    getLeagueRepository(userId).getBadges(userId),
  ])

  const me = members.find(m => m.me)
  if (!me) redirect('/leagues')
  ...
  const initialHistory: PredictionHistoryItem[] = items.map(p => {
    const m = p.match
  ...
  const initialNextCursor = hasMore ? (items.at(-1)?.id ?? null) : null
