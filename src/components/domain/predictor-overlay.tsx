...
function PredictorInner({ match }: { match: FeedMatch }) {
  ...
  if (locked) {
    return (
      <>
        <div className="sheet-backdrop" onClick={closePredictor} />
        ...
        >
          ...
          >
            ...
            >
              ...
              bonus <b style={{ color: "var(--warn)" }}>+3 pts</b> si aciertas
              (máx +5)
            </div>
          </div>
        </div>
...
export function PredictorOverlay() {
  const { predictorMatch } = useAppStore();
  if (!predictorMatch) return null;
  return <PredictorInner key={predictorMatch.id} match={predictorMatch} />;
}
